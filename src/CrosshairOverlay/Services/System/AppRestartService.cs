...
public sealed class AppRestartService : IAppRestartService
{
    ...
    public bool RestartWithFactoryReset(out string? error)
    {
        ...
        try
        {
            ...
            startInfo.ArgumentList.Add(StartupArguments.FactoryResetSwitch);
        ...
        {
            _logger.LogError(ex, "Không khởi động lại được để khôi phục cài đặt gốc.");
            error = ex.Message;
            return false;
        }

        _logger.LogWarning("Khôi phục cài đặt gốc: đã mở instance mới, instance này thoát.");
        error = null;
        _shutdown();
        return true;
    }
